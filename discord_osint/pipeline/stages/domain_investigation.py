...
Bug 5 fix: SSL certificate PEM is now encoded to bytes before being
           passed to OpenSSL.crypto.load_certificate.
"""
...
class DomainInvestigationStage(Stage):
    ...
    def _get_ssl_info(domain: str) -> dict:
        """Bug 5 fix: PEM must be bytes for OpenSSL.crypto.load_certificate."""
        import ssl
        try:
            ...
            cert = crypto.load_certificate(
                crypto.FILETYPE_PEM, cert_pem.encode("utf-8")
            )
