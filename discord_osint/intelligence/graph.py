...
Edges   – created by five co-occurrence rules:
...
Change log
----------
Rule 5 used to connect *every* pair inside a source bucket.  With ~300
discovery-sourced profiles that produced 40k+ edges and made the graph
62% dense — correlations became meaningless because everything was
connected to everything.  It is now capped to buckets with at most
``_CO_OCCURRENCE_MAX_BUCKET`` entities so small interesting sources
still connect but large scanner batches do not explode the graph.
"""
...
# Rule 5 is only applied to source buckets of this size or smaller.
# Anything bigger is a scanner batch — pairing every item in it would
# add O(n^2) edges and no real signal.
_CO_OCCURRENCE_MAX_BUCKET = 8
...
def build_graph(entities: list[BaseEntity]) -> Any:
    ...
    # --- Rule 5: Weak co-occurrence for same source (bounded) ---
    source_buckets: dict[str, list[BaseEntity]] = {}
    for ent in entities:
        if ent.source:
            source_buckets.setdefault(ent.source, []).append(ent)

    for _src, bucket in source_buckets.items():
        # Skip massive scanner batches — pairing them adds O(n^2) edges
        # for zero additional signal.
        if len(bucket) < 2 or len(bucket) > _CO_OCCURRENCE_MAX_BUCKET:
            continue
